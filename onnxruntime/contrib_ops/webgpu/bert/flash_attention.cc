...
Status FlashAttentionProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  // Use a larger K tile (32) to halve K loop iterations and reduce barrier sync overhead.
  // Only enabled for NVIDIA and Apple GPUs with head_size <= 128, which have enough registers.
  // SHM: max_k_step * (head_size/4) * 8bytes * 2(K+V). head_size=128: 32*32*8*2=16KB (OK)
  const uint32_t max_k_step = (qkv_head_size_ <= 128 && (is_nvidia_ || is_apple_)) ? 32 : 16;
